...
export default function Sale() {
  ...
  const fetchBooks = () => {
    baseApi
      .get(
        `books?size=${size}&sortOption=desc&sortByCol=promotionValue&index=${index}`,
        alert.current
      )
      .then((res: any) => {
  ...
  return (
    <>
      <div
        className={`alert alert-card mt-5 ${effect ? "effect-bunny" : ""}`}
      >
        <div className="row mt-4 mb-2">
          <div className="col medium">
            <h3 className=" mb-3 title">
              <i className="fa-solid fa-bolt" onClick={getEffect}></i> Sale
            </h3>
          </div>
        </div>
        <div className="row mt-2 mb-4">
          <div className="col-md-6">
            <button className="button mb-2" onClick={handleBack}>
        ...
        <div className="row mt-4 mb-2">
          {books ? (
                        ...
                        {book.promotionValue > 0 && (
                          <>
                            ...
                            <h3 className="tag-2 mt-3  ">
                              {" "}
                              {formatMoney(
                                (book.price * (100 - book.promotionValue)) / 100
                              )}{" "}
                            </h3>
                          </>
                        )}
            ...
            <h3 className="text-center medium">
              {" "}
              Loading... <i className="fa-solid fa-spinner effect-fan " />
            </h3>
          )}
        </div>
      </div>
