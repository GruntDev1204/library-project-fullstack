...
export default function Popular() {
  ...
  const fetchBooks = () => {
    baseApi
      .get(
        `books?size=${size}&sortOption=desc&sortByCol=transactionVolume&index=${index}`,
        alert.current
      )
      .then((res: any) => {
  ...
  return (
    <>
      <div className={`alert alert-card mt-5 ${turnOnFan ? "effect-fan" : ""}`}>
        <div className="row mt-4 mb-2">
          <div className="col medium">
            <h3 className=" mb-3 title">
              <i className="fa-solid fa-star" onClick={getEffect}></i> Popular
            </h3>
          </div>
        </div>
              ...
              <div key={book.id} className="col-md-4">
                <div className="alert flex-col-center card-content">
                  <div className="header-card">
                    {book.transactionVolume > 0 && (
                      <h3 className="mb-3 flex-end tag small">
                        dealt : {book.transactionVolume}
                      </h3>
                    )}
